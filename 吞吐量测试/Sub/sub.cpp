...
void UserDataTypeListener::on_data_available(DataReader* reader)
{
	...
	//}
	//cout<<data_seq.length()<<endl;
   
       /* 每秒显示吞吐量和丢包率 */
       if (elapsed_time >= 1.0) {
           //cout<<count_<<endl;
           double throughput = count_ * data_size * 8 / 1e6; // Mbps                  
	    ...
	    current_num  = pUserDataType->numpackes;
	   }
	   double loss_rate = 1 - static_cast<double>(count_)/(current_num - start_num); 
	   count_=0;
